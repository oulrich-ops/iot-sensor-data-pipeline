...
# Détection alertes avec cause précise (alert_type)
df_alerts = (
    df_parsed
    .withColumn(
        "alert_type",
...
# Fonction pour écrire dans PostgreSQL (table alerts)
def write_alerts_to_postgres(batch_df, batch_id):
    try:
        ...
        (
            ...
            .option("dbtable", "alerts")  # ta table alerts
...
# Stream vers PostgreSQL avec foreachBatch
query = (
    df_alerts.writeStream
    .foreachBatch(write_alerts_to_postgres)
    .outputMode("append")   # append pour les événements d’alertes
    .start()
)
